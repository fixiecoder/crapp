...
const optionConfig = {
  ...
  f: {
    ...
    options: [
      ...
      { def: '-f, --functional-component', param: 'functional-component', desc: 'add functional component file' },
...
function run() {
  ...
  switch(program.args[0]) {
    ...
    case 'f':
     if(program.args[1]) {
        const options = optionConfig.f.options.filter(option => program[option.param] === true).map(option => option.param);
        createFiles(program.args[1], options);
      } else {
